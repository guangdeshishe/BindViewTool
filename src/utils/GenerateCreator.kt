...
import createBindViewFieldByElement
...
//                    generateFindViewById()
     ...
     * 生成BindView类型变量
     */
    private fun generateBindViewCode() {

        generateBindViewFields()
     ...
     * 创建BindView变量
     */
    private fun generateBindViewFields() {
        if (mIsLayoutInflater) {
            val inflater = "private View $mLayoutInflaterText;"
            // 已存在的变量就不创建
            var duplicateField = false
        ...
        for (element in mElements) {
            ...
            if (element.isEnable) {
                // 添加到class
                mClass.add(mFactory.createFieldFromText(mProject.createFieldText(element, mFile).createBindViewFieldByElement(
                        element, mIsLayoutInflater, mLayoutInflaterText, mLayoutInflaterType), mClass))
            }
        }
    }
