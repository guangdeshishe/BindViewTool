...
open class GenerateDialog(
    ...
    private val mParamName = JLabel(Constant.Dialog.TABLE_FIELD_PARAM_NAME)
    private val mComments = JLabel(Constant.Dialog.TABLE_FIELD_COMMENTS)
    // 命名JPanel
    ...
    private val mNeedCastsCheck = JCheckBox(Constant.Dialog.NEED_CASTS, false)
    ...
    internal fun initTopPanel() {
        mPanelTitle.layout = GridLayout(1, 5, 10, 0)
        mPanelTitle.border = EmptyBorder(5, 5, 5, 5)
        mPanelTitleField.layout = GridLayout(1, 1, 0, 0)
        mTitleName.horizontalAlignment = JLabel.LEFT
        mTitleName.border = EmptyBorder(0, 0, 0, 0)
        ...
        mParamName.horizontalAlignment = JLabel.LEFT
        mComments.horizontalAlignment = JLabel.LEFT
        // 添加到JPanel
...
//        mPanelTitle.add(mPanelTitleField)
        mPanelTitle.add(mParamName)
        mPanelTitle.add(mComments)
//        mPanelTitle.setSize(100, 30)
        // 添加到JFrame
...
//        contentPane.add(mPanelNeedCasts, 4)
        contentPane.add(mPanelButtonRight)
    }
    ...
    internal fun initContentPanel() {
        ...
        for (i in elements.indices) {
            ...
            val itemJPanel = IdBean(GridLayout(1, 5, 10, 0),
                    EmptyBorder(0, 5, 0, 5),
                    JCheckBox(element.name),
                    JLabel(element.id),
                    JCheckBox("点击事件"),
                    JTextField(element.fieldName),
                    JTextField(element.comments),
                    element.isEnable,
                    element.isClickable,
            ...
            itemJPanel.setCommentsFocusListener { commentsJTextField -> element.comments =  commentsJTextField.text}
