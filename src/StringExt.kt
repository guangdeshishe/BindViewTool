...
fun String.firstToUpperCase(): String {
    ...
    return this.substring(0, 1).toUpperCase(Locale.CHINA) + this.substring(1)
}
 ...
 * @return String
 */
fun String?.createBindViewFieldByElement(element: Element, mIsLayoutInflater: Boolean, mLayoutInflaterText: String, mLayoutInflaterType: Int): String {
    val fromText = StringBuilder()
//    this?.let {
//        fromText.append("/** $it */\n")
//    }
    fromText.append("private @BindView(R.id." + element.id + ") ")
    if (element.isClickable) {
        fromText.append("@BindClick ")
    }
    with(element) {
        fromText.append("$name $fieldName")
    }
    if (mIsLayoutInflater) {
        fromText.append(layoutInflaterType2Str(mLayoutInflaterText, mLayoutInflaterType))
    }
    fromText.append(";")
    if (!element.comments.isNullOrEmpty()) {//添加注释
        fromText.append("//" + element.comments)
    }
    fromText.append("\n")
    return fromText.toString()
}
